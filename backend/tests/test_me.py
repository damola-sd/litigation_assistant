"""
Tests for GET /me — the stub auth dependency.
...
async def test_me_reflects_custom_user_id(client):
    r = await client.get("/me", headers={"x-user-id": "alice-test"})
    assert r.status_code == 200
...
async def test_me_different_users_return_different_ids(client):
    r_alice = await client.get("/me", headers={"x-user-id": "alice"})
    r_bob = await client.get("/me", headers={"x-user-id": "bob"})
